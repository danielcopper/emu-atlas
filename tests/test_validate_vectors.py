...
def _granularity(**overrides) -> Vector:
    return {
        ...
        "alternatives": [
            {"mode": "off", "options": {"opt": "off"}, "values": ["per-game-file"], "caveats": []}
        ],
        **overrides,
    }
...
PLACEMENT_CASES = [
    ...
    case(_base_placement(
             granularity=_granularity(
                 alternatives=[{"mode": "off", "options": {"opt": "off"}, "values": ["nope"], "caveats": []}]
             )
         ),
    ...
    case(_base_placement(
             granularity=_granularity(
                 alternatives=[{"mode": "off", "options": {"opt": "off"}, "values": ["per-game-file"],
                                "caveats": [{"code": "not-a-caveat", "data": {}}]}]
             )
         ),
         "caveat code must be one of", id="granularity-alternative-caveat-code"),
]
...
CATALOGUE_CASES = [
    ...
    case(_base_catalogue(entries=[_emulator(emulator="")]),
         "catalogue entry emulator must be null or a non-empty string", id="emulator-empty-identity"),
    case(_base_catalogue(entries=[_emulator(emulator=None)]),
         "the core a launch loads is what identifies it", id="emulator-identity-missing-on-a-core"),
    case(_base_catalogue(entries=[_emulator(emulator="mgba")]),
         "the core a launch loads is what identifies it", id="emulator-identity-disagrees-with-the-core"),
...
FIRMWARE_CORE_CASES = [
    ...
    case(_base_firmware(cores=[_core(emulator="")]),
         "firmware core emulator must be null or a non-empty string", id="core-empty-identity"),
    case(_base_firmware(cores=[_core(emulator=None)]),
         "the core a launch loads is what identifies it", id="core-identity-missing"),
    case(_base_firmware(cores=[_core(declared_index="0")]),
         "firmware core declared_index must be null or an integer >= 0", id="core-declared-index-type"),
    case(_base_firmware(cores=[_core(declared_index=-1)]),
         "firmware core declared_index must be null or an integer >= 0", id="core-declared-index-negative"),
...
REQUIREMENT_CASES = [
    ...
    case(_base_firmware(cores=[_core(requirements=[_requirement(declared_kind="folder")])]),
         "declared_kind must be one of", id="requirement-declared-kind-unknown"),
    # A file where the core lists a folder reaches nothing, and that is
    # established — the all-clear a file with no packaged identity would
    # otherwise carry is exactly what this rule refuses.
    case(_base_firmware(cores=[_core(requirements=[_requirement(declared_kind="directory", found="file",
    ...
    case(_base_firmware(cores=[_core(requirements=[_requirement(found="directory", present=True,
                                                                checked="unknown", satisfied=False)],
                                     requirements_met=False)]),
         "a directory where the core reads a file establishes nothing",
...
ACCEPTED_CASES = [
    ...
    pytest.param(_base_firmware(cores=[_core(requirements=[_requirement(declared_kind="directory",
                                                                        found="directory", present=True,
                                                                        checked="unknown", satisfied=None)],
                                             requirements_met=None)]),
                 id="a-folder-at-a-folder-declaration"),
    pytest.param(_base_firmware(cores=[_core(requirements=[_requirement(declared_kind="directory",
                                                                        found="file", present=True,
                                                                        checked="unknown", satisfied=False)],
                                             requirements_met=False)]),
                 id="a-file-where-the-core-opens-a-folder"),
    pytest.param(_base_firmware(hash_checked=True,
                                cores=[_core(requirements=[_requirement(declared_kind="directory",
                                                                        found="directory", present=True,
                                                                        checked="unknown", satisfied=True)],
                                             requirements_met=True)]),
                 id="a-folder-holding-a-recognised-image"),
    pytest.param(_base_firmware(cores=[_core(requirements=[_requirement(declared_kind="directory",
                                                                        found="directory", present=True,
                                                                        checked="unknown", satisfied=False)],
                                             requirements_met=False)]),
                 id="a-folder-holding-nothing-of-an-accepted-size"),
    pytest.param(_base_firmware(hash_checked=True,
                                cores=[_core(requirements=[_requirement(found="file", present=True,
                                                                        identity=_identity(),
                                                                        checked="verified", satisfied=True)],
    ...
    pytest.param(_base_catalogue(entries=[_emulator(kind="standalone", core_so=None, label="Citra",
                                                    emulator=None)]),
                 id="an-entry-whose-command-identifies-no-emulator"),
    pytest.param(_base_firmware(cores=[_core(declared_index=1)]),
                 id="a-firmware-core-built-from-a-catalogue-row"),
