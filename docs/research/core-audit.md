...
| ----------------------- | ---------------- | ------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| flycast                 | Flycast          | **card**           | flycast@1dac369: `libretro.cpp:790-807` maps the live option to `per_content_vmus`; reachable `oslib.cpp:38-65` builds a game-ID/content-name VMU path, called by `maple_devs.cpp:409-428`; shared `bios/dc` VMUs observed live, exact per-game identity mapping still [O]                                                                                                                                                         |
...
| opera                   | Opera            | **card**           | opera-libretro source (`opera_lr_nvram.c:117-175`): NVRAM under `<save_dir>/opera/per_game/<stem>.<version>.srm` (default) or `<save_dir>/opera/shared/nvram.<version>.srm`; `RETRO_MEMORY_SAVE_RAM` is NULL (`libretro.c:574-579`) — the nvram files are the only persistence. Not yet live-observed                                                                                                                              |
| genesis_plus_gx         | Genesis Plus GX  | **multi-option**   | GPGX source: cart SRAM via the libretro memory interface (`libretro.c:3746-3765`) = standard `.srm`. Sega-CD BRAM is core-written into the save dir with names governed by three options (`system_bram` default `per bios` → shared `scd_<region>.brm`; `cart_bram` default `per cart` + `cart_size` default `4meg` → shared `4Mbit_cart.brm`; both switchable to per-game `<stem>[_<size>].brm`, `libretro.c:1383-1500`)          |
| neocd                   | NeoCD            | **multi-option**   | neocd source: core writes backup RAM itself (`path.cpp:137-168`: default `<system_dir>/neocd/neocd.srm` shared; `neocd_per_content_saves=On` → `<save_dir>/<stem>.srm`) **and** exposes the same RAM via `RETRO_MEMORY_SAVE_RAM` (`libretro.cpp:214-215`) — RetroArch persists a standard `.srm` in parallel. Which copy wins on load is unestablished [O]; a card stating one location would guess — deferred until live-verified |
...
whether per-game saves are a proven capability, and — per arrangement (retrodeck / emudeck / bare) — the versions the
knowledge was verified against (`null` = never verified there).
...
(`docs/tasks/save-detection.md`). A shared default can therefore coexist with `per_game_capable: true`.
