...
8. ~~`puae` / `puae2021`~~ — read 2026-09-05, both carded. The `cd32nvram` hit is the emulated chip's own config key,
   ...
   file at close, `puae_floppy_write_protection` discards every write and outranks it. Seven modes over the floppy and
   CD classes; archives, playlists, WHDLoad and hard-disk content are outside the record and the rule refuses them,
   because an archive's or a playlist's save story is decided by what is inside the file and WHDLoad's `WHDSaves` tree
   is a follow-up's subject.
