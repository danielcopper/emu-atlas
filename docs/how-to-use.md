...
list is a group of its own where every entry found was decided**, and a caveat lists them: a machine with two accounts
gets two trees rather than a guess at which is in force. With an entry the survey could not decide beside them — the two
ways that happens are below — the caveat says instead that the accounts stated are the ones established here, and names
the undecided entries. It never says the listing could have stopped early, which is where this differs from the Vita3K
sentence: RPCS3's walk skips an entry whose `fstatat` fails and reads on into the next, so an undecided entry costs the
emulator's own list that entry at most. Where the tree holds no user home at all, the caveat says exactly that — the
directory named is the one the emulator creates at startup, not one found here — and a tree that could not be listed
carries `save-dir-unlistable` of its own, so "which users exist is unknown" is something a client can branch on. Two
more things ride along: the per-title directories keep their names refused (they are the games' own), and a _second_
place saves live rides as its own group — `savedata/vmc`, the virtual memory cards for PS1 and PS2 classics, outside the
per-user tree entirely, with `files: null` because what lands there has not been read, and `file-set-spans-roots` saying
a sync that walks only the per-user tree misses it. That card is also the first to read YAML, through a reader that
...
way. A name below `home` whose **own** `stat` failed is stated under `skipped` where `check_user` rejects the name —
that test reads nothing but the name, so a name it rejects is passed over whichever way the walk went — and under
`unestablished` otherwise, where it is not a skip at all: the walk that would reach it stats every entry and drops the
ones whose `fstatat` fails (unix_dir::read, File.cpp:2091-2105), so the emulator's verdict rests on a stat of its own,
which is exactly what atlas did not get. Where every entry found is passed over and none is unestablished, `reason` says
no account the emulator would list was found — not that no directory was; where at least one entry found is one atlas
could not decide instead, whatever else was passed over, `reason` says whether the emulator would list a user account
here was not established. Vita3K's rule is the `user.xml` load described above (get_users_list, user_management.cpp:89),
and the same three fates apply: a directory whose `user.xml` loads is a group, one without a `user.xml` or with one that
does not parse is stated under `skipped`, and one whose `user.xml` atlas could not read is stated under `unestablished`;
the recorded-user check then runs over that list. An entry whose own `stat` failed is `unestablished` here as well, for
a reason the library settles rather than leaves open: `get_users_list` tests `fs::is_directory` on a
`boost::filesystem::directory_entry`, which answers from the kind the walk cached out of `readdir`'s `d_type` and stats
...
Boy Advance that needs a BIOS — over that machine, only a GBA BIOS is. A declaration like that one — boot images for the
Game Boy, the Game Boy Color, the Super Game Boy and the Game Boy Advance — resolves to as many systems as the per-file
override table files those names under, and that table files the Super Game Boy dumps under `snes`, because the
cartridge runs in an SNES rather than in a Game Boy. The entries the table of systems has for those machines are
recorded `open`, so nothing about them turns on the state yet. Where a core in this state declares for one system the
conjunction has one term, its disjunction spans the whole declaration, and the field reads as it always did — which is
every core reaching that state measured so far: 61 core blocks in the vector corpus, all for one system, counted over
every core block in `vectors/machines/*.json` whose `system_firmware` is `cannot-run-without-firmware` and, per block,
the distinct `system` values of its requirements and their alternatives. The count moves with the corpus and the
property does not, so the property is the one held mechanically: `tests/test_system_firmware.py` walks the same rule and
fails naming the block the day one declares for two systems. Which images a system's name covers is as fine as the
per-file override table makes it — a `.info` files every image under one `systemname`, so where nothing separates them
they are one system's images.
...
`BiosTools.cpp:325-335`), which reads the ROMDIR header (`LoadBiosVersion`, `BiosTools.cpp:60-173`) — no hash table
anywhere. It needs exactly one image: the first found becomes the `pcsx2_bios` option's default (`main.cpp:1832-1834`),
`LoadBIOS` opens one file (`BiosTools.cpp:281`), and when the configured one is absent `FindBiosImage` returns the first
...
| files of an accepted size, no `verify`                                                     | `None`                                       | `firmware-search-unverified` (`dir`, `candidates`, `need`, `core_so`) — a content question not asked for content is not answered "yes"; the header check is a content read, and none is made                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| an image by the header check whose bytes the table lists, `verify`                         | `True`                                       | one `firmware-image-identified` per such image, in path order (`path`, `image`, `md5`, `table`, `core_so`, `description`, `zone`, `version`, `date`, `serial`) — all of them, none picked: the core offers them all as option values, up to 127 — the fill loop stops one short of the 128-slot `RETRO_NUM_CORE_OPTION_VALUES_MAX` to leave room for the terminator (`main.cpp:1828`, `:1833`; `libretro.h:6394`)                                                                                                                                                                                                                                                                                                                           |
| an image by the header check whose bytes the table does not list, `verify`                 | `True`                                       | `firmware-image-unlisted` (`path`, `description`, `zone`, `version`, `date`, `serial`, `md5`, `table`, `core_so`) — the table lists what `System.dat` lists, a subset of what the check accepts, so this is an image the core offers, named by its own header; `md5` is the digest when taken and `""` when not, and `table` is the packaged table's version it is absent from when `md5` is non-empty — with `md5` empty the table was not consulted                                                                                                                                                                                                                                                                                       |
| an image by the header check whose bytes could not be hashed, `verify`                     | `True`                                       | `firmware-image-unlisted` with `md5` empty, worded without a table lookup nobody made, beside `firmware-unreadable` (`path`) for the digest that was not taken — a digest the seam could not produce is a read failure beside a header that came back, never bytes the table does not know; the header is the verdict and stands, and what the table calls the bytes stays unestablished                                                                                                                                                                                                                                                                                                                                                    |
| accepted sizes, `verify`, every one read and none passes the header check                  | `False`                                      | `firmware-directory-holds-no-image` (`dir`, `candidates`, `paths`, `core_so`, `table_version`) — each file was read the way the core reads it and fails the one content test the listing makes; `candidates` counts them and agrees with the same key on `firmware-search-unverified`, since every file of an accepted size was read, and `paths` names them, sorted and joined with `", "`, the shape `firmware-scan-incomplete` gives its `unreadable`; a rejected candidate whose digest could not be taken keeps this verdict and adds `firmware-unreadable` (`path`) beside it — a read failure beside a header that came back, stating that the one thing the digest could have added, a table hit the header denies, was not checked |
| a candidate whose bytes the table lists and whose header does not read as a BIOS, `verify` | `None`, unless another candidate is an image | `firmware-image-contradicted` (`path`, `image`, `md5`, `core_so`, `table`, `table_version`) — two reads over one file disagree, and neither is taken over the other                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| a candidate whose header read did not come back, `verify`                                  | as the rest                                  | `firmware-unreadable` (`path`) per such file — the ROMDIR read the core's own test makes did not come back, so nothing is stated about the file, whatever its digest matched; with nothing else passing the check the verdict is `None`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |

`image` is the table's own name for the content (`ps2-0200e-20040614.bin`) whatever the file is called: the core accepts
any name, so the name is by bytes. `table` is the packaged table's schema version (its `_meta.version`). `description`
is the label the core builds from the fourteen-byte `ROMVER` string and the `EXTINFO` serial and shows as the
`pcsx2_bios` option's label (`main.cpp:1819`, `:1829`; the format at `BiosTools.cpp:147-155` — the zone left-justified
in seven, `v` major.minor, the date as day/month/year, two spaces, `Console` or `Devel` or nothing, the serial), and
where the file is shorter than the walked table says, the core's truncation suffix follows (`BiosTools.cpp:165-170`), so
the field can end in `50%`; `zone` is the word the zone letter maps to (`BiosTools.cpp:117-133`), `version` is
`major.minor`, `date` is `year-month-day` in the version string's own order, and `serial` is the `EXTINFO` string, empty
where the walk met none. Several images are more choice, not a conflict and not "better". `requirements_met` follows
`satisfied` as it always has, and `hash_checked` says whether `verify` ran.
...
that passes the core's own header check, is `False`.

A file a folder declaration's read accounted for — an image by the header check, a candidate the table names and the
check denies, or one whose bytes it could not read — is claimed by that declaration and stays out of `unclaimed`, stated
