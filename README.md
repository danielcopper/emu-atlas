...
The conformance vectors (`vectors/`, schema 5) are whole fixture machines — files, directories, symlinks, AppImage
contents, core answers and the ways a core probe comes back empty, firmware blobs and read-failure states — each
replayed against the canonical serialization and asserted with exact equality, alongside the unit suite on every push.
Each release attaches the vectors, the wheel, the bundle and a `SHA256SUMS` manifest.
