...
4. **Standalone emulators** — per-emulator config parsing and save placement rules, read the way each emulator reads its
   own configuration: Dolphin (GC slots and the Wii NAND), PPSSPP, xemu and Cemu (the per-title MLC unit) today, one
   emulator per sub-issue of the epic (#3) until every emulator RetroDECK ships resolves — on EmuDeck too, where the
   catalogue launches them through launcher scripts instead of tokens.
