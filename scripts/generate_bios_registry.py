#!/usr/bin/env python3
"""Generate atlas/data/bios_registry.json from libretro-core-info and libretro-database.

This is the offline, dev-time build step for the packaged BIOS registry. It parses
libretro's ``.info`` files for firmware entries (path, opt, desc) and their
``systemname``, merges those with the hashes in libretro-database's ``System.dat``,
groups everything by platform slug (via ``SYSTEMNAME_TO_SLUG``), and writes the
result — the data file that ships inside the ``atlas`` package and that
``atlas.bios.load_registry`` reads.

The inputs are two local git checkouts passed as arguments; nothing here touches the
network. Clone them first, then run the generator against the checkouts:
...
With no ``-o`` the output defaults to ``atlas/data/bios_registry.json`` (resolved
relative to the repo root, so the command works from any working directory). A
regeneration is a deliberate, reviewable data diff — see ``atlas/data/README.md``.
...
DEFAULT_OUTPUT = REPO_ROOT / "atlas" / "data" / "bios_registry.json"
...
def main():
    ...
    parser.add_argument(
        ...
        help="Output JSON path (default: atlas/data/bios_registry.json)",
    )
